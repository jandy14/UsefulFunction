...
Shader "Unlit/ForceField"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				fixed4 noise = tex2D(_MainTex, i.uv + float2(_Time.x,-_Time.x * 1.1) );
				...
				float hitted = 0;
				for (int index = 0; index < _HittedPosSize; ++index)
				{
					hitted += smoothstep(_HittedPosArray[index].w, 0, length(i.worldPos.xyz - _HittedPosArray[index].xyz));
				}
				hitted = saturate(hitted);
				//float surf = step(0.7, noise);
				float surf = 0;
				fixed4 col = fixed4(1, 0, 0, saturate(0.1 + 0.1 * frac(i.worldPos.y * 10 + _Time.y) + rim + inter + hitted + surf));
				return col;
			}
			ENDCG
		}
